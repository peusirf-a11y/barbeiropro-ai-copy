...
export default function ComplianceRetention() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3 bg-blue-500/10 border border-blue-500/30 rounded-xl p-3 text-xs text-blue-500">
          ...
          <div key={p.id} className="bg-card rounded-2xl border border-border p-4 shadow-sm">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex-1 min-w-[180px]">
                ...
                <div className="text-[11px] text-muted-foreground mt-0.5">{p.note}</div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                ...
                <div className="flex gap-1 flex-wrap">
                  {PRESETS.filter(pr => pr.unit === p.unit || p.unit === 'hours').slice(0, p.unit === 'hours' ? 1 : 5).map(pr => (
                    <button key={pr.label} onClick={() => update(p.id, 'value', pr.value)}
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full border transition-colors ${p.value === pr.value && p.unit === pr.unit ? 'bg-blue-500 text-white border-blue-500' : 'bg-muted text-muted-foreground border-border hover:border-foreground/30'}`}>
                      {pr.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-1.5 bg-muted rounded-lg px-2.5 py-1.5 border border-border">
                  <input type="number" min="1" value={p.value} onChange={e => update(p.id, 'value', e.target.value)}
                    className="w-14 text-sm font-bold text-foreground bg-transparent focus:outline-none text-right" />
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleSave}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${saved ? 'bg-emerald-500 text-white' : 'bg-blue-500 text-white hover:bg-blue-600'}`}>
