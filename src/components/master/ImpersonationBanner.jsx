...
export default function ImpersonationBanner() {
  ...
  return (
    <div className="sticky top-0 z-40 bg-amber-500 text-white px-4 py-2 flex items-center justify-between gap-3 text-sm">
      <div className="flex items-center gap-2 min-w-0">
        <Eye className="w-4 h-4 flex-shrink-0" />
        <span className="truncate">
          Visualizando como <strong>{imp.company_name}</strong> · expira em ~{expiresIn}min
        </span>
      </div>
      <button
