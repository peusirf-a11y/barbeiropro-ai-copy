  ...
  return (
    <div className="bg-white rounded-2xl border border-black/8 p-5">
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <h3 className="font-bold text-[#1B1C1E]">Histórico de envios</h3>
        <div className="flex gap-2">
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
            className="text-xs px-3 py-1.5 border border-black/10 rounded-lg bg-white">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-10 text-sm text-gray-400">Nenhum envio registrado</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-gray-500 border-b border-black/5">
            ...
            <tbody>
              {logs.map(log => {
                ...
                return (
                  <tr key={log.id} className="border-b border-black/5 hover:bg-gray-50">
                    <td className="py-2.5 pr-3 text-xs text-gray-500 whitespace-nowrap">
                      {ts ? format(new Date(ts), "dd/MM HH:mm", { locale: ptBR }) : '—'}
                    </td>
                    <td className="py-2.5 pr-3 text-xs text-gray-700">{TYPE_LABELS[log.type] || log.type}</td>
                    <td className="py-2.5 pr-3 text-xs text-gray-700 truncate max-w-[200px]" title={log.recipient}>{log.recipient}</td>
                    <td className="py-2.5 pr-3">
                      <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded ${cfg.color}`}>
                        <Icon className="w-3 h-3" /> {cfg.label}
                      </span>
                    </td>
                    <td className="py-2.5 pr-3 text-xs text-red-600 font-mono truncate max-w-[280px]" title={log.error_message}>
                      {log.error_message || '—'}
                    </td>
                  </tr>
                );
