    ...
    <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-[var(--shadow-sm)]">
      <div className="p-4 sm:p-5 border-b border-border flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h2 className="font-bold text-foreground text-lg tracking-tight">Alertas do sistema</h2>
          {unreadCount > 0 && (
      ...
      <div className="divide-y divide-border max-h-[480px] overflow-y-auto">
        ...
        {!isLoading && alerts.length === 0 && (
          <div className="p-12 text-center">
            ...
            <div className="text-sm font-semibold text-foreground">Nenhum alerta. Tudo certo!</div>
          </div>
        )}
        {alerts.map(a => {
          ...
          return (
            <div
              key={a.id}
              className={`p-4 flex items-start gap-3 transition-colors ${!a.read ? 'bg-amber-500/10' : 'hover:bg-muted/40'}`}
            >
              ...
              {!a.read && (
                <button
                  ...
                  className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg flex-shrink-0 transition-colors"
