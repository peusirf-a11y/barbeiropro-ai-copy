...
const SEV_STYLE = {
  critical: { dot: 'bg-red-500',    badge: 'bg-red-500/15 text-red-500 border-red-500/30',    label: 'CRÍTICO' },
  high:     { dot: 'bg-orange-500', badge: 'bg-orange-500/15 text-orange-500 border-orange-500/30', label: 'ALTO' },
  medium:   { dot: 'bg-amber-400',  badge: 'bg-amber-500/15 text-amber-500 border-amber-500/30',   label: 'MÉDIO' },
  low:      { dot: 'bg-blue-400',   badge: 'bg-blue-500/15 text-blue-500 border-blue-500/30',      label: 'BAIXO' },
};
...
function KpiCard({ label, value, icon: Icon, color = 'blue', urgent }) {
  ...
  return (
    <div className={`bg-card rounded-2xl border p-4 shadow-sm ${urgent ? 'border-red-500/40 ring-1 ring-red-500/20' : 'border-border'}`}>
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Eventos de segurança · Brute force · Cross-tenant · LGPD</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => refetch()} className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-xl text-sm hover:bg-muted text-foreground bg-card">
            <RefreshCw className="w-3.5 h-3.5" /> Atualizar
          </button>
          <button onClick={exportCSV} className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-xl text-sm hover:bg-muted text-foreground bg-card">
      ...
      <div className="flex gap-1 bg-muted rounded-xl p-1">
        {[
          ...
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`flex items-center gap-1.5 flex-1 text-sm font-semibold py-2 px-3 rounded-lg transition-all ${activeTab === t.key ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-3">
          {/* Filters */}
          <div className="bg-card rounded-2xl border border-border p-3 shadow-sm flex flex-wrap gap-2 items-center">
            <Filter className="w-3.5 h-3.5 text-muted-foreground" />
            <select value={filterSev} onChange={e => setFilterSev(e.target.value)}
              className="px-2 py-1.5 text-xs border border-border rounded-lg focus:outline-none bg-background text-foreground">
            ...
            <span className="ml-auto text-[11px] text-muted-foreground">{filtered.length} eventos</span>
          </div>

          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
            {isLoading ? (
              <div className="p-10 text-center text-muted-foreground text-sm">Carregando eventos…</div>
            ) : filtered.length === 0 ? (
              <div className="p-10 text-center">
                <CheckCircle2 className="w-8 h-8 text-emerald-500/60 mx-auto mb-2" />
                <p className="text-sm text-muted-foreground">Nenhum evento detectado. Sistema saudável.</p>
              </div>
            ) : (
              <div className="divide-y divide-border max-h-[500px] overflow-y-auto">
                {filtered.map(ev => {
                  ...
                  return (
                    <div key={ev.id} className="px-4 py-3 hover:bg-muted/40 transition-colors">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            <span className="text-[13px] font-semibold text-foreground">{EVENT_LABELS[ev.event_type] || ev.event_type}</span>
                            {!ev.blocked && <span className="text-[10px] text-amber-500 font-bold">NÃO BLOQUEADO</span>}
                          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-border flex items-center gap-2">
              ...
              <span className="font-bold text-sm text-foreground">IPs bloqueados agora</span>
            </div>
            {rateLimits.length === 0 ? (
              <div className="p-4 text-center text-muted-foreground text-xs">Nenhum IP bloqueado.</div>
            ) : (
              <div className="divide-y divide-border max-h-48 overflow-y-auto">
                {rateLimits.slice(0, 20).map(r => (
                  <div key={r.id} className="px-4 py-2 flex items-center justify-between gap-2">
                    <div>
                      <div className="text-[12px] font-mono text-foreground">{r.ip || r.identifier || '—'}</div>
                      <div className="text-[10px] text-muted-foreground">{r.route} · {r.attempts} tentativas</div>
                    </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-border flex items-center gap-2">
              ...
              <span className="font-bold text-sm text-foreground">Score de risco por tenant</span>
            </div>
            <div className="divide-y divide-border max-h-64 overflow-y-auto">
              {tenantScores.map(co => (
                <div key={co.id} className="px-4 py-2.5 flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-xs font-black ${co.score >= 80 ? 'bg-emerald-500/15 text-emerald-500' : co.score >= 50 ? 'bg-amber-500/15 text-amber-500' : 'bg-red-500/15 text-red-500'}`}>
                    {co.score}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] font-semibold text-foreground truncate">{co.name}</div>
