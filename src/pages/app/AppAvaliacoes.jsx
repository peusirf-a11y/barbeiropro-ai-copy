    ...
    <AppLayout>
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl">
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">Aprove avaliações antes que elas fiquem visíveis publicamente</p>
        </div>
        ...
        {published.length > 0 && (
          <div className="bg-white rounded-2xl border border-black/8 p-6 mb-5 grid sm:grid-cols-2 gap-6 items-center">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-400">{published.length} avaliação{published.length > 1 ? 'ões' : ''} publicada{published.length > 1 ? 's' : ''}</div>
            </div>
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${
                  isActive ? 'bg-[#2563EB] text-white' : 'bg-white border border-black/10 text-gray-600 hover:border-[#2563EB]'
                }`}>
                {t.label}
                ...
                <div key={r.id} className="p-5">
                  <div className="flex items-center justify-between mb-2 gap-3 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${r.published ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
