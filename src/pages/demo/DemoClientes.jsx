/**
 * DemoClientes — Paridade TOTAL com AppClientes.
 * Mesma lógica de filtros, contadores, badges e modal.
 */
...
export default function DemoClientes() {
  ...
  // Mesmo mapa do AppClientes: customer_id => assinatura ativa
  const subByCustomer = useMemo(() =>
    demoSubscriptions
      .filter(s => s.status === 'active')
      .reduce((acc, s) => { acc[s.customer_id] = s; return acc; }, {})
  , []);

  // Mesmo getCustomerStats do AppClientes: conta atendimentos concluídos
  const getCustomerStats = (customerId) =>
    demoAppointments.filter(a => a.customer_id === customerId && a.status === 'concluido').length;

  // Mesmos contadores de filtro do AppClientes
  const counts = useMemo(() => customers.reduce((acc, c) => {
    if (c.status === 'vip') acc.vip++;
    if (c.lifecycle_status) acc[c.lifecycle_status] = (acc[c.lifecycle_status] || 0) + 1;
    return acc;
  }, { vip: 0, primeira_visita: 0, fiel: 0, em_risco: 0, inativo: 0, perdido: 0 }), [customers]);

  // Mesma lógica de filtro do AppClientes
  ...
  const handleSave = () => {
    if (editing) {
      ...
      setCustomers(prev => [...prev, {
        ...
        lifecycle_status: 'primeira_visita',
      }]);
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {[
              ...
              >
                <span>{f.l}</span>
                {f.count > 0 && (
                  <span className={`text-[10px] font-bold ${filter === f.v ? 'text-white/80' : 'text-gray-400'}`}>
                    {f.count}
                  </span>
                )}
              </button>
        ...
        <div className="bg-white rounded-2xl border border-black/5 overflow-hidden shadow-[var(--shadow-sm)]">
          {filtered.length > 0 ? (
                    ...
                    <tr key={c.id} className="border-b border-black/5 hover:bg-[#FAFBFC] transition-colors">
                      ...
                      <td className="p-4 hidden lg:table-cell text-sm text-[#6B7280]">
                        {c.last_appointment_at
                          ? format(new Date(c.last_appointment_at), "d MMM yyyy", { locale: ptBR })
                          : '–'}
                      </td>
                      <td className="p-4">
            ...
            <div className="p-16 text-center text-[#6B7280]">
              ...
              {!search && (
                <button onClick={() => setShowForm(true)} className="text-sm font-semibold text-[#2563EB] mt-2 hover:underline">
        ...
        <StandardModal
          ...
          footer={
            <>
              <button
                onClick={closeForm}
                className="flex-1 min-h-[48px] px-4 border border-black/10 rounded-xl text-sm font-medium text-[#111827] hover:bg-gray-50 active:bg-gray-100"
              >
                Cancelar
              </button>
              <button
