// Automação entity (FinancialEntry.create type=entrada) → registra "first_payment".
...
Deno.serve(async (req) => {
  try {
    ...
    if (data.type !== 'entrada') return Response.json({ success: true, skipped: 'not_income' });
    ...
    await sdk.entities.UserEvent.create({
      ...
      event_type: 'first_payment',
      source: 'automation',
      metadata: { entry_id: data.id, amount: data.amount },
    });
    return Response.json({ success: true, tracked: true });
  } catch (error) {
    console.error('[onFinancialEntryCreatedEvent]', error.message);
