...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ANTES de qualquer chamada Stripe (cara) ou criação de Appointment.
    // Atacante reciclando idempotency keys ainda é barrado por IP.
    const ipRl = await _checkIpRateLimit(sdk, ip, 'createBookingPaymentIntent');
    if (!ipRl.allowed) {
      console.warn(`[createBookingPaymentIntent] rid=${rid} IP_RATE_LIMITED ip=${ip} reason=${ipRl.reason}`);
      await _logRateLimitEvent(sdk, { ip, route: 'createBookingPaymentIntent', reason: ipRl.reason, attempts: ipRl.attempts, rid });
      return Response.json({
        error: 'rate_limited',
    ...
    const {
      ...
      idempotency_key,
      customer_id: authenticatedCustomerId, // Fase 9: opcional. Quando vier do AuthGate, vira o owner do lock.
    } = body;
    ...
    // Fase 9: se o frontend mandou customer_id (fluxo autenticado), passamos como
    // owner do lock — match estrito impede phone-spoofing roubar slot. Quando não
    // vier (fluxo legado / convidado), caímos no fallback por telefone.
