...
async function getCallerContext(base44, user, impersonation_token) {
  ...
  if (impersonation_token && user.is_super_admin) {
    ...
    console.log('[listWhatsAppMessages] impersonation', { actor: user.email, company_id: company.id });
...
Deno.serve(async (req) => {
  try {
    ...
    const { active_unit_id, customer_id, type, status, impersonation_token } = body || {};
